...
(
  ~multi: bool,
  ~arrowRenderer: arrowRendererProps => React.element=?, /* Renders a custom drop-down arrow. Won't render when set to null */
  ...
  ~clearRenderer: unit => React.element=?, /* Renders a custom clear to be shown in the right-hand side of the select when clearable true: clearRenderer() */
  ...
  ~inputRenderer: Js.t({..}) => React.element=?, /* renders a custom input component */
